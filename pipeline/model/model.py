...
import os
import re
from pipeline.utils.losses import l2
from pipeline.utils.losses import acc as acc_function
...
class Model:
                ...
                for i, batch in enumerate(train_loader):
                    ...
                    for cb in self.callbacks:
                        cb.per_batch({
                            ...
                            'y_true': y_true,
                            'acc': acc,
                            'loss_xn': loss_function(y_pred, x)
                        })

                    test_loss_ += \
                        (
                                loss.item() +
                                loss_function(y_pred, x).item()
    ...
    def predict(self,
        ...
        raise RuntimeError("Don\'t implement predict method")
