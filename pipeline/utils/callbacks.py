...
class VisImageForFFTAE(AbstractCallback):
    def __init__(self, title, server='https://localhost', port=8080,
                 vis_step=1, scale=10, fft_shape=None):
        self.viz = Visdom(server=server, port=port)
    ...
    def per_batch(self, args, label=1):
        if self.n % self.step == 0:
            ...
            for win in self.windows.keys():
                if win == label:
                    input_fft_tensor = args['x'][i].detach().to(
                        'cpu').numpy()
                    output_fft_tensor = args['y_pred'][i].detach().to(
                        'cpu').numpy()
                    expected_fft_tensor = args['y_true'][i].detach().to(
                        'cpu').numpy()

                    input_fft_tensor = np.vectorize(complex)(
                        input_fft_tensor[0],
                        input_fft_tensor[1]
                    )
                    output_fft_tensor = np.vectorize(complex)(
                        output_fft_tensor[0],
                        output_fft_tensor[1]
                    )
                    expected_fft_tensor = np.vectorize(complex)(
                        expected_fft_tensor[0],
                        expected_fft_tensor[1]
                    )

                    input_image = \
                        torch.FloatTensor(np.fft.irfft2(input_fft_tensor))
                    output_image = \
                        torch.FloatTensor(np.fft.irfft2(output_fft_tensor))
                    expected_fft_tensor = \
                        torch.FloatTensor(np.fft.irfft2(expected_fft_tensor))

                    x = torch.cat(
                        (input_image, output_image, expected_fft_tensor),
                        dim=1
                    )
